...
<div class="card">
  ...
  <div class="card-body">
      <form class="producto" method="POST" action="{{route('producto.store')}}">
 ...
 {{$errors}}
                     ...
                     <h3>Datos del producto</h3>
                        ...
                        <input type="text" id="txtCompania" class="form-control" value="{{old('Compania')}}" name="Compania" placeholder="Ingresa la Compañia"  required   onKeyPress="return soloLetras(event)" onkeyup="soloLetras(this);">
                        <p class="inputError">{{ $errors->first('Compania') }}</p>
                    </div>  

                      <div class="form-group col-md-4">
                        <label for="txtModelo">Modelo</label>
                        <input type="text" id="txtModelo" class="form-control" value="{{old('Modelo')}}" name="Modelo" placeholder="Ingresa el modelo"  required   onKeyPress="return soloLetras(event)" onkeyup="soloLetras(this);">
                        <p class="inputError">{{ $errors->first('Modelo') }}</p>
                    </div>       

                    <div class="form-group col-md-4">
                        <label for="txtAnio">Año del auto</label>
                        <input type="text" id="txtAnio" class="form-control" value="{{old('Anio')}}" name="Anio" placeholder="Ingresa el año del auto"  required   onKeyPress="return soloNumeros(event)" onkeyup="soloNumeros(this);">
                    ...
                    <div class="form-group col-md-4">
                            ...
                            <select class="form-control form-control-chosen-required {{$errors->first('Version') ? 'has-error' : ''}}" id="txtVersion" placeholder="Ingrese la version del auto" name="Version" required >
                             @if(!old('Version') || old('Version') == 'Elige...')  <option selected>Elige...</option> @endif
                              ...
                              <option value="0" {{old('Version') == '0' ? 'selected' : ''}}>Sedan</option>
                              <option value="1" {{old('Version') == '1' ? 'selected' : ''}}>Comfortline</option>
                              <option value="2" {{old('Version') == '2' ? 'selected' : ''}}>Live</option>
                              <option value="3" {{old('Version') == '3' ? 'selected' : ''}}>Highline</option>
                            </select>
                         ...
                         <div class="form-group col-md-4">
                            ...
                            <select class="form-control form-control-chosen-required {{$errors->first('Color') ? 'has-error' : ''}}" id="txtColor" placeholder="Ingrese el color del auto" name="Color" required >
                             @if(!old('Color') || old('Color') == 'Elige...')  <option selected>Elige...</option> @endif
                              ...
                              <option value="0" {{old('Color') == '0' ? 'selected' : ''}}>Azul</option>
                              <option value="1" {{old('Color') == '1' ? 'selected' : ''}}>Rojo</option>
                              <option value="2" {{old('Color') == '2' ? 'selected' : ''}}>Negro</option>
                              <option value="3" {{old('Color') == '3' ? 'selected' : ''}}>Gris plata</option>
                              <option value="4" {{old('Color') == '4' ? 'selected' : ''}}>Blanco</option>
                            </select>
                            <p class="inputError">{{ $errors->first('Version') }}</p>
                        </div>
